...
- **Ask what changed on the device, not only in the diff.** The first question is output route, OS
  version, network path — `docs/device-check.md` §0 now opens with it.
- **An instrument that has not itself been checked is not evidence.** `ui/audio-check.html` was
  written to settle this and its first version `await`ed `ctx.resume()` before starting the
  oscillator, which puts the start outside the user activation WebKit requires — so it reported
  silence on a device that was working, and a false diagnosis was built on that. It is a standalone
  page that imports nothing from the app, which is what makes it useful: it runs when the app does
  not. Confirm Tone D sounds repeatedly before believing anything else it says.
